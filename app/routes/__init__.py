...
def send_mail(recieptant,course_name):
    ...
    msg.body = f'Hello you have successfully enrolled in {course_name}. Happy learning !!!'
    mail.send(msg)
    # flash('Sent')
    return "sent"
# send mail complete
        ...
        try:
            ...
            if dt:
                if bcrypt.checkpw(ps1, dt1):
                    ...
                    session['wrongCred'] = "Please enter correct username and password"
                    return redirect('/login')
            else:
                session['wrongCred'] = "User not registered"
                return redirect('/login')
        except Exception as ex:
            print(ex)
            session['wrongCred'] = "User not registered"
            return redirect('login')
    wrongCred = session.get('wrongCred')
    return render_template('auth/login.html', form=form, wrongCred=wrongCred)
...
def register_user():
    ...
    if form.validate_on_submit():
        ...
        try:
            user = Profile(profile_photo=filename, name=form.name.data, designation=form.designation.data,
                           contact_no=form.contact_no.data, username=form.username.data, password=hashed_password)
            db.session.add(user)
            db.session.commit()
...
def course():
    ...
    return render_template('pages/course.html', user=user, page=page, courses=courses, word = word, worddata = worddata)
...
def enroll_course():
    ...
    try:
        ...
        # flash(cDate)
            ...
            db.session.commit()
            mailsent = send_mail(recieptant.email,course.name)
            if mailsent:
